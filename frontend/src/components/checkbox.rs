...
impl Component for Checkbox {
    ...
    fn create(ctx: &Context<Self>) -> Self {
        ...
        let indeterminate = ctx.props().indeterminate;

        Checkbox { checked, indeterminate }
    }

    fn changed(&mut self, ctx: &Context<Self>, _old_props: &Self::Properties) -> bool {
        self.checked = ctx.props().checked;
        self.indeterminate = ctx.props().indeterminate;
            ...
            <span class={classes!(base_class, class)}>
                ...
                <svg class={classes!("select-none", "w-[1em]", "h-[1em]", "inline-block", "fill-current", "shrink-0", "text-2xl", "transition-colors")} focusable="false" aria-hidden="true" viewBox="0 0 24 24">
                    {if self.checked {
                        ...
                        html!(<path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-2 10H7v-2h10v2z"></path>)
                    } else {
